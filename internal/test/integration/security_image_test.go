...
func TestSecurityImageIntegration(t *testing.T) {
	...
	// create a SecurityImage adapter
	securityImageDB := database.NewSecurityImage(dbConn.Queries)
	...
	t.Run("SaveAndRetrieveSecurityImage", func(t *testing.T) {
		securityImage := "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQ..."
		...
		saveResp, err := securityImageDB.SaveSecurityImage(ctx, saveReq)
		require.NoError(t, err)
		...
		getResp, err := securityImageDB.GetSecurityImage(ctx, getReq)
		require.NoError(t, err)
	...
	t.Run("UpdateExistingSecurityImage", func(t *testing.T) {
		...
		_, err := securityImageDB.SaveSecurityImage(ctx, saveReq1)
		require.NoError(t, err)
		...
		_, err = securityImageDB.SaveSecurityImage(ctx, saveReq2)
		require.NoError(t, err)
	...
	t.Run("NonExistentUser", func(t *testing.T) {
		...
		_, err := securityImageDB.GetSecurityImage(ctx, getReq)
		assert.Error(t, err)
	})
	...
	t.Run("DirectDatabaseAccess", func(t *testing.T) {
		...
		_, err := securityImageDB.SaveSecurityImage(ctx, saveReq)
		require.NoError(t, err)
